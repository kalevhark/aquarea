...
class TuyaAPI:
  ...
  def get_log(self, device_id, hours=24):
    from datetime import datetime, timedelta
    now = datetime.now()
    endTime = int(now.timestamp() * 1000)
    startTime = int((now - timedelta(hours=hours)).timestamp() * 1000)
    return self._api(
      {"action": "tuya.m.smart.operate.all.log"},
      {
        "devId": device_id,
        "dpIds": "1",
        "offset": 0,
  ...
  def get_help(self, device_id):
    return self._api(
      {"action": "tuya.m.smart.list"},
      {
        "devId": device_id,
      }
    )
